...
public class FileProvider extends ContentProvider {
    ...
    static XmlResourceParser getFileProviderPathsMetaData(Context context, String authority, ProviderInfo info, int resourceId) {
        ...
        XmlResourceParser in2 = info.loadXmlMetaData(context.getPackageManager(), META_DATA_FILE_PROVIDER_PATHS);
        if (in2 == null) {
    ...
    private static PathStrategy parsePathStrategy(Context context, String authority, int resourceId) throws IOException, XmlPullParserException {
        ...
        XmlResourceParser in2 = getFileProviderPathsMetaData(context, authority, info, resourceId);
        while (true) {
            int type = in2.next();
            if (type != 1) {
                if (type == 2) {
                    String tag = in2.getName();
                    String name = in2.getAttributeValue(null, ATTR_NAME);
