        ...
        while (true) {
            ...
            if (i2 < dArr.length) {
                ...
                if (motion != null) {
                    ...
                    float drx = vel[0];
                    float dry = vel[1];
                    float angle = path_rotate2;
                    float pos_x = (float) ((rx + (radius * Math.sin(v_y4))) - (v_width3 / 2.0f));
                    float pos_y = (float) ((ry - (radius * Math.cos(v_y4))) - (v_height2 / 2.0f));
                    double d = dradius;
                    v_width = v_width3;
                    v_height = v_height2;
                    float dpos_x = (float) (drx + (d * Math.sin(v_y4)) + (radius * Math.cos(v_y4) * dangle));
                    float dpos_y = (float) ((dry - (dradius * Math.cos(v_y4))) + (radius * Math.sin(v_y4) * dangle));
                    v_x = pos_x;
                    if (slope.length >= 2) {
                        slope[0] = dpos_x;
                        slope[1] = dpos_y;
                    ...
                    if (!Float.isNaN(angle)) {
                        double d2 = angle;
                        path_rotate = dpos_y;
                        float rot = (float) (d2 + Math.toDegrees(Math.atan2(dpos_y, dpos_x)));
                        motionWidget = view;
                        motionWidget.setRotationZ(rot);
                    } else {
                        motionWidget = view;
                        path_rotate = dpos_y;
                    }
                    ...
                    if (!Float.isNaN(path_rotate2)) {
                        float dx = dv_x2 + (dv_width / 2.0f);
                        float dy = dv_y2 + (dv_height3 / 2.0f);
                        float rot2 = (float) (0.0f + path_rotate2 + Math.toDegrees(Math.atan2(dy, dx)));
                        motionWidget.setRotationZ(rot2);
                    }
                    v_y = v_y5;
                }
                int l = (int) (v_x + 0.5f);
