...
public abstract class DispatchedTask<T> extends Task {
    ...
    public final void run() {
        ...
        try {
            ...
            undispatchedCompletion$iv = undispatchedCoroutine;
        } catch (Throwable e) {
            fatalException = e;
            try {
                ...
                result = Result.m98constructorimpl(Unit.INSTANCE);
            } catch (Throwable th) {
                th = th;
                Result.Companion companion2 = Result.Companion;
                result = Result.m98constructorimpl(ResultKt.createFailure(th));
                handleFatalException(fatalException, Result.m101exceptionOrNullimpl(result));
            }
        }
        try {
            ...
            try {
                if (job != null && !job.isActive()) {
                    ...
                    if (DebugKt.getRECOVER_STACK_TRACES()) {
                        try {
                            if (continuation instanceof CoroutineStackFrame) {
                                ...
                                continuation.resumeWith(Result.m98constructorimpl(ResultKt.createFailure(cancellationException)));
                            }
                        } catch (Throwable th3) {
                            th = th3;
                    ...
                    continuation.resumeWith(Result.m98constructorimpl(ResultKt.createFailure(exception)));
                } else {
                    T successfulResult$kotlinx_coroutines_core = getSuccessfulResult$kotlinx_coroutines_core(state);
                    Result.Companion companion5 = Result.Companion;
                    continuation.resumeWith(Result.m98constructorimpl(successfulResult$kotlinx_coroutines_core));
                }
